...
int main(void)
{
  ...
  //uint32_t save = grpc_hfxo_switch();
  BTL_DEBUG_INIT();
...
// uint32_t save = grpc_hfxo_switch();
  ...
  if (ret == BOOTLOADER_OK) {
    ...
    if (reset_getResetCounter() >= 2) {
      ...
      reset_invalidateResetReason();
      while (1) {
          BTL_DEBUG_PRINTLN("btl_main boot app failure, loop ...");
        // Wait...
      }
    } else {
...
#endif
//  EUSART->CFG2
//  uint32_t reg = EUSART1->CFG2;


 // BTL_DEBUG_PRINT_CHAR_HEX ((reg>>24) & 0xff);

  BTL_DEBUG_PRINTLN("EnterApp...");
